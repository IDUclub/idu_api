...
class UrbanMCPConfig:
    ...
    @classmethod
    def example(cls) -> "UrbanMCPConfig":
        """Generate an example of configuration."""

        return cls(
            app=AppConfig(
                uvicorn=UvicornConfig(host="0.0.0.0", port=8001, reload=True, forwarded_allow_ips=["127.0.0.1"]),
                debug=True,
            ),
            db=MultipleDBsConfig(
                master=DBConfig(
    ...
    @classmethod
    def load(cls, file: str | Path | TextIO) -> "UrbanMCPConfig":
        ...
        try:
            return UrbanMCPConfig._initialize_from_dict(UrbanMCPConfig, data)
    ...
    def _initialize_from_dict(t: Type, data: Any) -> Any:
        ...
        if get_origin(t) is Union or get_origin(t) is UnionType:  # both actually required
            for inner_type in t.__args__:
                if inner_type is NoneType and data is None:
                    return None
                try:
                    return UrbanMCPConfig._initialize_from_dict(inner_type, data)
        ...
        for fld in fields(t):
            ...
            if inner_data is None:
                ...
                init_dict[fld.name] = UrbanMCPConfig._initialize_from_dict(fld.type, inner_data)
        return t(**init_dict)

    @classmethod
    def from_file(cls, config_path: str) -> "UrbanMCPConfig":
