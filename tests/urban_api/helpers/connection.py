...
class MockConnection:
    ...
    def __init__(self, preset_results: list[MockResult] = None, *args, **kwargs):
    ...
    async def execute(self, query, paging_data=None):
        ...
        return self._get_next_result(query, paging_data)
    ...
    def _get_next_result(self, query, paging_data=None):
        if self._result_index < len(self.preset_results):
            result = self.preset_results[self._result_index]
            self._result_index += 1
            return result

        # fallback to autogenerated result
        columns = self._get_query_columns(query)
        data = {col: self._mock_value(dtype) for col, dtype in columns.items()}
