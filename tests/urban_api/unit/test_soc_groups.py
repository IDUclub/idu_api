...
async def add_service_type_to_social_group_to_db(
    ...
    # Act
    result = await add_service_type_to_social_group_to_db(mock_conn, soc_group_id, soc_group_service_type_post_req)
...
async def test_get_social_value_by_id_from_db(mock_conn: MockConnection):
    ...
    # Act
    with patch(
        "idu_api.urban_api.logic.impl.helpers.soc_groups.check_existence", new=AsyncMock(side_effect=check_soc_value)
    ):
        with pytest.raises(EntityNotFoundById):
    ...
    assert isinstance(SocValue.from_dto(result), SocValue), "Couldn't create pydantic model from DTO."
    mock_conn.execute_mock.assert_any_call(str(statement))
...
async def test_put_social_value_indicator_value_to_db(
    ...
    statement = (
        ...
        .on_conflict_do_update(
            ...
            set_=soc_value_indicator_put_req.model_dump(),
        )
        .returning(soc_value_indicators_data)
    )
...
async def test_delete_social_value_indicator_value_from_db(mock_conn: MockConnection):
    ...
    statement = delete(soc_value_indicators_data).where(
        soc_value_indicators_data.c.soc_value_id == soc_value_id,
