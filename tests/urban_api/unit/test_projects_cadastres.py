...
from idu_api.urban_api.dto import UserDTO, ProjectCadastreDTO
from idu_api.urban_api.exceptions.logic.projects import NotAllowedInRegionalProject
from idu_api.urban_api.logic.impl.helpers.projects_cadastres import get_cadastres_by_project_id_from_db, \
    put_project_cadastres_to_db, delete_cadastres_by_project_id_from_db
...
async def test_put_project_cadastres_to_db(
    ...
    insert_statement = (
        insert(projects_cadastres_data)
        .values([{"project_id": project_id, **extract_values_from_model(project_cadastre_put_req)}])
    )
